...
public class ProgrammingLanguageSupport {
  ...
  // Java構文パターン
  private static final String[] JAVA_PATTERNS =
      new String[] {
  ...
  // Go構文パターン
  private static final String[] GO_PATTERNS =
      new String[] {
  ...
  // C/C++構文パターン
  private static final String[] CPP_PATTERNS =
      new String[] {
  ...
  // Haskell構文パターン
  private static final String[] HASKELL_PATTERNS =
      new String[] {
  ...
  /** パターンに基づくハイライト計算 */
  private StyleSpans<Collection<String>> computeHighlightingForPatterns(
      String text, String[] patterns, String prefix) {
  ...
  private String getStyleClassForMatch(Matcher matcher, String prefix) {
    if (matcher.group(prefix + "KEYWORD") != null) return "keyword";
    if (matcher.group(prefix + "STRING") != null) return "string";
    if (matcher.group(prefix + "COMMENT") != null) return "comment";
    if (matcher.group(prefix + "NUMBER") != null) return "number";
    if (prefix.equals("CPP") && matcher.group("CPPPREPROCESSOR") != null) return "preprocessor";
    if (prefix.equals("HASKELL") && matcher.group("HASKELLOPERATOR") != null) return "operator";
    return "plain";
  }

  /** プログラミング言語ハイライト用CSSを取得 */
  public String getProgrammingLanguageCSS() {
    return ".keyword { -fx-fill: #0000ff; -fx-font-weight: bold; } "
        + ".string { -fx-fill: #008000; } "
        + ".comment { -fx-fill: #808080; -fx-font-style: italic; } "
        + ".number { -fx-fill: #ff8000; } "
        + ".preprocessor { -fx-fill: #800080; } "
        + ".operator { -fx-fill: #ff0000; } "
        + "/* ダークテーマ用 */ "
