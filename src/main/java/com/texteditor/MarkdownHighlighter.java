...
public class MarkdownHighlighter {
	...
	// Markdown構文の正規表現パターン
	private static final String[] MARKDOWN_PATTERNS = new String[]{"(?<HEADING1>^#{1}\\s+.*)$", // # 見出し1
			"(?<HEADING2>^#{2}\\s+.*)$", // ## 見出し2
			"(?<HEADING3>^#{3}\\s+.*)$", // ### 見出し3
	...
	private static final Pattern MARKDOWN_PATTERN = Pattern.compile(String.join("|", MARKDOWN_PATTERNS),
			Pattern.MULTILINE);
	...
	private String getStyleClass(Matcher matcher) {
		...
		if (matcher.group("BOLD") != null)
			return "bold";
		if (matcher.group("ITALIC") != null)
			return "italic";
		if (matcher.group("CODE") != null)
			return "code";
		if (matcher.group("CODEBLOCK") != null)
			return "codeblock";
		...
		if (matcher.group("LIST") != null)
			return "list";
		if (matcher.group("NUMLIST") != null)
			return "numlist";
		if (matcher.group("STRIKETHROUGH") != null)
			return "strikethrough";
		if (matcher.group("HORIZONTAL") != null)
			return "horizontal";
	...
	public String getMarkdownHighlightCSS() {
		return ".heading1 { -fx-font-size: 20px; -fx-font-weight: bold; -fx-fill: #2E74B5; } "
				...
				+ ".link { -fx-fill: #0366d6; -fx-underline: true; } " + ".image { -fx-fill: #28a745; } "
				+ ".list { -fx-fill: #323130; } " + ".numlist { -fx-fill: #323130; } "
